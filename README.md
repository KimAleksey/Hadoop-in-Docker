...
## 📖 План обучения и Теория
В файле `learning_plan.md` находится детальный роадмап для изучения технологий.
А в файле **[GUIDE.md](GUIDE.md)** — подробный учебник по устройству этого проекта (архитектура, компоненты, примеры).
...
## 🐘 Подключение через DBeaver (SQL)
Теперь вы можете работать с Big Data как с обычной базой данных!

1. **Скачайте DBeaver** (Community Edition).
2. **Создайте новое соединение**:
   - **Тип драйвера**: Лучше всего выбрать **Apache Spark** (т.к. мы подключаемся к Thrift Server).
   - Если его нет выберите **Apache Hive** (версия 2.x или 3.x).
   - **Host**: `localhost`
   - **Port**: `10000`
   - **Database**: `default`
   - **Username**: `hive` (или любой другой)
   - **Password**: `hive` (или пустой)
3. **Нажмите "Test Connection"**.
   - DBeaver предложит скачать драйверы — соглашайтесь.
4. **Готово!** Теперь вы можете писать SQL запросы к вашим данным в HDFS.

**Пример создания таблицы:**
_Выполняйте команды по одной (выделите строку и нажмите Ctrl+Enter):_
